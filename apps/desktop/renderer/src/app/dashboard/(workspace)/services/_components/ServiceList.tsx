    ...
    },
    {
      ...
      cell: (item) => (
        <div>
          ...
          <div className="text-muted-foreground text-xs">SAC: {item.hsnCode || '-'}</div>
        </div>
      ),
    },
    {
      ...
      cell: (item) => (
        <div className="flex justify-end gap-1">
          <AppButton
            variant="ghost"
            size="sm"
            onClick={() => router.push(`/dashboard/services/view?id=${item.id}`)}
            title="Edit"
          >
  ...
  return (
    <div className="bg-background flex h-full flex-col overflow-hidden px-6 py-6">
      <div className="mb-6 flex items-start justify-between">
        <SectionHeader title="Services" description="Manage your services." />
      </div>
      ...
      <div className="flex-1 overflow-auto">
        <DataTable
          ...
          rowTestIdExtractor={(item) => `service-row-${item.name}`}
          isLoading={isLoading}
          ...
          toolbar={{
            ...
            placeholder: 'Search by Name, SKU, or SAC',
            actions: (
              <AppButton
