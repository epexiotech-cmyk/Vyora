...
export function SupplierForm({ initialData, isEditMode = false }: SupplierFormProps) {
  ...
  const [supplierType, setSupplierType] = React.useState<'INDIVIDUAL' | 'ENTITY'>('ENTITY');
  ...
  const onSubmit: SubmitHandler<CreateSupplierInput> = async (data) => {
    try {
      ...
      const payload = {
        ...
        contactPerson: supplierType === 'INDIVIDUAL' ? cleanedData.name : cleanedData.contactPerson,
      };
          ...
          >
            ...
            <AppCard className="p-6 shadow-sm">
              ...
              <div className="mb-4 flex items-center gap-6 border-b px-2 pb-4">
                <label className="flex cursor-pointer items-center gap-2">
                  <input
                    ...
                    name="supplierType"
                    value="ENTITY"
                    checked={supplierType === 'ENTITY'}
                    onChange={() => setSupplierType('ENTITY')}
                  />
              ...
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <AppField
                  name="name"
                  label={supplierType === 'INDIVIDUAL' ? 'Supplier Name *' : 'Company/Firm Name'}
                >
                  <FormInput
                    data-testid="supplier-name-input"
                    name="name"
                    type="text"
                    placeholder={supplierType === 'INDIVIDUAL' ? 'e.g. John Doe' : 'e.g. Acme Corp'}
                  />
                </AppField>
                {supplierType === 'ENTITY' && (
