  ...
  public async getPosBook(
    ...
    if (startDate) {
      const priorFilter: JournalQueryFilter = {
        companyId,
        financialYearId,
        ledgerId, // Strict ledgerId filter to ONLY get pos movements
    ...
    // 3. Query all voucher entries for the requested period involving the pos ledger
    ...
    const voucherMap = new Map<string, PosBookVoucherDto>();
    const orderedVouchers: PosBookVoucherDto[] = [];

    for (const entry of periodEntries) {
      if (!voucherMap.has(entry.voucherId)) {
        const newVoucher: PosBookVoucherDto = {
    ...
    for (const voucher of orderedVouchers) {
      ...
      // Only movements belonging to the selected Pos ledger contribute
...
export const posBookService = new PosBookService();
